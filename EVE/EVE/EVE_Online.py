...
class EVE:
    def __init__(self):
        ...
        self.system_behavior = """
            Your name is EVE, which stands for Expert Virtual Engineer.
        ...
        self.get_weather_func = types.FunctionDeclaration(
            ...
            parameters=types.Schema(
                type=types.Type.OBJECT, properties={"location": types.Schema(type=types.Type.STRING,
                    ...
                    async for response in session.receive():
                        try:
                            ...
                            if response.tool_call:
                                ...
                                print(
                                    f"--- Received Tool Call: {tool_call_name} with args: {tool_call_args} (ID: {tool_call_id}) ---")
                                ...
                                if tool_call_name in self.available_functions:
                                    ...
                                    try:
                                        ...
                                        print(
                                            f"--- Sending Tool Response for {tool_call_name} (ID: {tool_call_id}) ---")
...
async def main():
    print("Starting EVE Assistant...")
    eve = EVE()

    if eve.pya is None or eve.recorder is None:
        print("Failed to initialize audio components. Exiting.")
        return

    # Create tasks for each concurrent operation
    tasks = [
        asyncio.create_task(eve.stt()),  # Speech to Text -> input_queue
        asyncio.create_task(eve.send_prompt()),  # input_queue -> Gemini (handles tools) -> response_queue
        asyncio.create_task(eve.tts()),  # response_queue -> ElevenLabs -> audio_queue
        asyncio.create_task(eve.play_audio()),  # audio_queue -> Speaker
        # asyncio.create_task(eve.input_message()) # Optional: Uncomment for text input instead of STT
    ]
    ...
    try:
        ...
        if eve.pya:
            ...
            await asyncio.to_thread(eve.pya.terminate)  # Clean up PyAudio resources
